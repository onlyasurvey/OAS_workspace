...
%>
<title>View Role - <c:out value="${subject.displayTitle}" />
...
<p>
	...
	(<a href="viewApplication.html?id=<c:out value="${application.id}"/>"><c:out value="${application.displayTitle}" /></a>)
</p>
<p>
...
<p>
	User List (<c:out value="${fn:length(list)}"/> entries)
</p>
...
<tbody>
	...
	<tr>
		...
		<td><a href="revokeRole.html?actorId=<c:out value="${item.id}"/>&amp;roleId=<c:out value="${subject.id}"/>">Revoke</td>
	</tr>
	</c:forEach>
</tbody>
</table>
