...
<c:set var="showChoices" value="false"/>
<c:set var="choiceSelectOptions">
	...
	<c:if test="${subject.choiceQuestion}">
		...
		<c:forEach items="${subject.choices}" var="choice">
			<%-- this will get called a lot, but it ensures we never show options if none exist --%>
			<c:set var="showChoices" value="true"/>
...
<form action="<c:url value='/html/db/mgt/qbr/aenr/${question.id}.html'/>" method="post">
<%--
