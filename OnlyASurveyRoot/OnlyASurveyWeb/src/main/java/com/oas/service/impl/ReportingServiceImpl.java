...
import com.oas.controller.dashboard.report.CalendarBreakdownController;
...
public class ReportingServiceImpl extends AbstractServiceImpl implements ReportingService {
	...
	/**
	 * Used by {@link CalendarBreakdownController}-type reports to query the
	 * backend.
	 * 
	 * @param clazz
	 *            Return type
	 ...
	 *            Subject
	 * @param startDate
	 *            Date to start query from
	 * @param maxResults
	 *            Maximum number of results
	 * @return
	 */
	private <C extends AbstractResourceModel> List<C> doCalendarBreakdownQuery(final Class<C> clazz, final Survey survey,
			final Date startDate, final int maxResults) {
		...
		List<C> list = (List<C>) getHibernateTemplate().execute(new HibernateCallback() {
			...
			public Object doInHibernate(Session session) throws HibernateException, SQLException {
				...
				Query query = session.createQuery(
						"from " + clazz.getSimpleName() + " where id.survey = ? and id.date >= ? order by id.date desc")
	...
	@SuppressWarnings("unchecked")
	private <C extends AbstractResourceModel> Map<Long, C> getPerDateMap(Class<C> clazz, int calendarField, int numEntries,
			Survey survey, Calendar calendar) {
		...
		for (int i = 0; i < numEntries; i++) {
			...
			C item = null;
			try {
				item = (C) BeanUtils.instantiateClass(clazz.getConstructor(new Class[] { Survey.class, Date.class, Long.class }),
						new Object[] { survey, thisDate, 0L });
	...
	public Collection<ResponsesPerDay> getResponseRatePerDay(final Survey survey, final int numDays) {
		...
		// execute the query
		List<ResponsesPerDay> list = doCalendarBreakdownQuery(ResponsesPerDay.class, survey, startDate, numDays);
	...
	public Collection<ResponsesPerMonth> getResponseRatePerMonth(final Survey survey, final int numMonths) {
		...
		// execute the query
		List<ResponsesPerMonth> list = doCalendarBreakdownQuery(ResponsesPerMonth.class, survey, startDate, numMonths);
	...
	public MonthlyReport getMonthlyReport(Survey survey) {
		...
		Collection<ScaleCalendarBreakdown> scaleBreakdown = find("from ScaleMonthlyBreakdown where survey = ?", report
				.getSurvey());
	...
	public Map<Question, Collection<TextAnswer>> getTextAnswers(Survey survey) {
		...
		Collection<TextAnswer> valueList = find("from TextAnswer where response.survey = ? AND " + //
				"response.closed = true AND response.deleted = false " + //
				"order by response.created", survey);
	...
	public Collection<TextCalendarValue> getTextResponsesPerDay(Question question, Date date) {
		...
		Collection<TextCalendarValue> retval = find(
				"from TextDailyValue where id.question = ? and id.month = ? order by id.response.created", args);
		return retval;
	}
	...
	public Collection<TextCalendarValue> getTextResponsesPerMonth(Question question, Date date) {
		...
		Collection<TextCalendarValue> retval = find(
				"from TextMonthlyValue where id.question = ? and id.month = ? order by id.response.created", args);
		return retval;
	}
	...
	@Override
	@ValidUser
	public List<PartialResponseHighestQuestionSummary> getAbandonmentHighestQuestionSummary(Survey survey) {
		List<PartialResponseHighestQuestionSummary> retval = find(
				"from PartialResponseHighestQuestionSummary where id.survey = ? order by id.questionIndex", survey);
		return retval;
	}
	...
	@Override
	@ValidUser
	public List<TimeTakenToComplete> getTimeTakenData(Survey survey) {
		List<TimeTakenToComplete> retval = find("from TimeTakenToComplete where id.survey = ? order by id.minutes", survey);
		return retval;
	}
