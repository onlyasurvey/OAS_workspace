...
public class PreferencesControllerTest extends AbstractPreferencesRelatedTest {
	...
	// private void doSubmitAndExpectRedirect(PreferencesCommand command, String
	// expectedRedirectUrl) throws Exception {
	//
	// ModelAndView mav = controller.doSubmit(command, new
	// BindException(command, "command"), new MockHttpServletRequest(),
	...
	// ModelAndView mav = controller.doSubmit(command, new
	// BindException(command, "command"), request,
	// new MockHttpServletResponse());
	// assertNotNull("no mav", mav);
	...
	// PreferencesCommand command = newValidCommand();
	// try {
	// controller.doSubmit(command, new BindException(command, "command"), new
