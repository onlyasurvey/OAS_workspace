...
public class SurveyServiceTest extends AbstractOASBaseTest {
	...
	public void cloneQuestion_Success_Choice_SelectList() {
		...
		question.setStyle(QuestionTypeConstants.STYLE_SELECT);
	...
	public void countResponses() {
		...
		// finds all responses that are deleted, but may be open or closed -
		// test data has 1 and 4, respectively
		assertEquals("unexpected # [non-deleted, open-or-closed responses]", Integer.valueOf(5), surveyService
				.countResponses(survey));
	}

	@Test
	public void countClosedResponses() {
		Survey survey = scenarioDataUtil.createTypicalScenario1(createAndSetSecureUserWithRoleUser(), true);
		scenarioDataUtil.addHistoricalResponseData(survey);
		flushAndClear();

		// finds all responses that are deleted, but MUST be closed -
		// test data has 1 and 4, respectively
		assertEquals("unexpected # [non-deleted, closed] responses", Integer.valueOf(4), surveyService
				.countClosedResponses(survey));
	}
	...
	private void addTestLogos(Survey survey, SupportedLanguage language) {
		//
	...
	public void getLogosForLanguage_Success() {
		...
		addTestLogos(survey, getEnglish());

		flushAndClear();

		Map<SurveyLogo.PositionType, SurveyLogo> map = surveyService.getLogosForLanguage(survey, getEnglish());
	...
	public void purgeLogo_Success() {
		...
		// purge
		for (SurveyLogo.PositionType position : new SurveyLogo.PositionType[] { SurveyLogo.PositionType.LEFT,
				SurveyLogo.PositionType.RIGHT }) {
			surveyService.purgeLogo(load(Survey.class, survey.getId()), getEnglish(), position);
			Map<SurveyLogo.PositionType, SurveyLogo> map = surveyService.getLogosForLanguage(survey, getEnglish());
			SurveyLogo loaded = map.get(position);
			assertIsNull("logo not purged: " + position, loaded);
			flushAndClear();
		}

		// purge non-existent logo should not cause method to fail
		for (SurveyLogo.PositionType position : new SurveyLogo.PositionType[] { SurveyLogo.PositionType.LEFT,
				SurveyLogo.PositionType.RIGHT }) {
			try {
				surveyService.purgeLogo(load(Survey.class, survey.getId()), getEnglish(), position);
				Map<SurveyLogo.PositionType, SurveyLogo> map = surveyService.getLogosForLanguage(survey, getEnglish());
				SurveyLogo loaded = map.get(position);
				assertIsNull("not supposed to exist: " + position, loaded);
				flushAndClear();
			} catch (Exception e) {
				throw new RuntimeException("purging null logo should NOT throw exception", e);
			}
		}
