...
<div>
	<label for="msgtext"><fmt:message key="inviteNew.messageField" /></label>
	<textarea name="message" cols="75" rows="15" id="msgtext"><c:if test="${empty command.message}"><fmt:message key="inviteNew.defaultMessage" /></c:if><c:if test="${not empty command.message}"><c:out value='${command.message}' /></c:if>
</textarea>
