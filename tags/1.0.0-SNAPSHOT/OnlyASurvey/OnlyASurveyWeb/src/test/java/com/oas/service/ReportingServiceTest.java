...
public class ReportingServiceTest extends AbstractOASBaseTest {
	...
	public void testGetResponseRatePerDay_Success() {
		...
		assertEquals("should have 1 day worth of data for today", 1, reportingService.getResponseRatePerDay(survey, 1).size());
		assertEquals("should have 2 days worth of data for 2 days", 2, reportingService.getResponseRatePerDay(survey, 2).size());
		assertEquals("should have 3 days worth of data for 3 days", 3, reportingService.getResponseRatePerDay(survey, 3).size());

		// code will fill in any dates without data with zeros, test
		assertEquals("should have 10 days worth of data for 10 days", 10, reportingService.getResponseRatePerDay(survey, 10)
				.size());
	}
	...
	public void testGetResponseRatePerMonth_Success() {
		...
		assertEquals("should have 1 month worth of data for this month", 1, reportingService.getResponseRatePerMonth(survey, 1)
				.size());
		assertEquals("should have 2 months worth of data for 2 months", 2, reportingService.getResponseRatePerMonth(survey, 2)
				.size());
		assertEquals("should have 3 months worth of data for 3 months", 3, reportingService.getResponseRatePerMonth(survey, 3)
				.size());

		// code will fill in any dates without data with zeros, test
		assertEquals("should have 10 months worth of data for 10 months", 10, reportingService
				.getResponseRatePerMonth(survey, 10).size());
	}
	...
	public void testGetResponsesPerLanguage_Success() {
		...
		for (int i = 0; i < 3; i++) {
			persist(new Response(survey, new Date(), supportedLanguageService.findByCode("eng"), LOCALHOST_IP));
		}

		// add 2 French responses
		for (int i = 0; i < 2; i++) {
			persist(new Response(survey, new Date(), supportedLanguageService.findByCode("fra"), LOCALHOST_IP));
		}
		flushAndClear();

		Map<SupportedLanguage, Integer> list = reportingService.getResponsesPerLanguage(survey);
	...
	public void testDailyReport() {
		calendarReportImpl(new CalendarReportImplCallback() {
			...
			public CalendarReport getCalendarReport(Survey survey) {
				return reportingService.getDailyReport(survey);
			}
	...
	public void testMonthlyReport() {
		calendarReportImpl(new CalendarReportImplCallback() {
			@Override
			public CalendarReport getCalendarReport(Survey survey) {
				return reportingService.getMonthlyReport(survey);
			}
		}, Calendar.MONTH);
	}
	...
	public void testGetTextResponsesPerMonth_Success() {
		...
		for (Calendar calendar : calendarList) {
			Collection<TextCalendarValue> report = reportingService.getTextResponsesPerMonth(textQuestion, calendar.getTime());
	...
	public void testGetTextResponses_Success() {
		...
		Map<Question, Collection<TextAnswer>> report = reportingService.getTextAnswers(survey);
		assertNotNull("report missing", report);
