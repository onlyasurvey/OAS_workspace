	...
	<tr> 
		...
		<c:forEach items="${report.dateList}" var="date">
			...
			<c:otherwise>
				...
				<c:if test="${countThisDate > 0}"><a href="<oas:url value='/html/db/rpt/txt/${question.id}.html?d=${dateParam}' />" rel="popup standard 550 300"><fmt:message key='monthlyBreakdown.view'/> (${countThisDate})</a></c:if>
				</td>
			</c:otherwise>
			</c:choose>
			<%-- TOTAL --%>
