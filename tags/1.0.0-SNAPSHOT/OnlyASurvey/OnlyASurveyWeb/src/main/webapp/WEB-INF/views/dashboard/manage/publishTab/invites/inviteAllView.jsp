...
<div>
	<label for="message"><fmt:message key="inviteAll.messageField" /></label>
	<textarea name="message" cols="75" rows="15" id="message"><c:if test="${empty command.message}"><fmt:message key="inviteAll.defaultMessage" /></c:if><c:if test="${not empty command.message}"><c:out value='${command.message}' /></c:if>
	</textarea>
</div>
