...
<title><fmt:message key="dashboardPage.title" /></title>
...
<p><a href="<oas:url value='/html/db/crt.html'/>"><fmt:message key='dashboardPage.createNewSurvey'/></a></p>
...
<table class="manageSurveysTable" summary="" border="0">
	...
	<c:set var="surveyName"><c:out value='${summary.survey.displayTitle}'/></c:set>
		...
		<tr>
			<td>
				<a class="surveyLink alignLeft" href="<oas:url value="/html/db/mgt/${summary.survey.id}.html"/>">${surveyName}</a>
			</td>
			<td>
				${summary.count}
			</td>
			...
			<%--
				...
				<a href="<oas:url value='/html/db/mgt/${summary.survey.id}.html'/>"><img class="icon" src="<oas:url value='/incl/images/icon-edit.gif'/>"
					alt="<fmt:message key='editSurveyIconAlt'/> ${surveyName}" title="<fmt:message key='editSurveyIconAlt'/> ${surveyName}" /></a>
			--%>
			<%--
				<img class="icon" src="<oas:url value='/incl/images/view_disabled.gif'/>"
			...
			--%>
				<c:if test="${summary.survey.published}">
					<a href='<oas:url value="/html/db/rpt/${summary.survey.id}.html"/>'><img class="icon" src="<oas:url value='/incl/images/icon-report.png'/>"
						alt="<fmt:message key='reportsIconAlt'/> ${surveyName}" title="<fmt:message key='reportsIconAlt'/> ${surveyName}" /></a>
				</c:if>
				<c:if test="${!summary.survey.published}">
					<img class="icon" src="<oas:url value='/incl/images/report_disabled.png'/>"
			...
			--%>
				...
				<a href='<oas:url value="/html/db/mgt/rm/${summary.survey.id}.html?rTo=/html/db/db.html"/>'><img class="deleteIcon" src="<oas:url value='/incl/images/icon-delete.gif'/>"
...
<ul>
	...
	<c:if test="${summary.survey.published}">
		<c:set var="surveyName"><c:out value='${summary.survey.displayTitle}'/></c:set>
		<li><a href="<oas:url value='/html/db/rpt/${summary.survey.id}.html'/>" title="<fmt:message key='reportsIconAlt'/> ${surveyName}">${surveyName}</a></li>
	</c:if>
	</c:forEach>
</ul>
</c:if>
<p><a href="<oas:url value='/html/db/rpt.html'/>">View All Reports</a></p>
</div>
<div class="yui-u">
<h2>Account Information</h2>

<p><strong>Account Type:</strong> Beta</p>
<p><strong>Published Survey Limit</strong>: Unlimited</p>
