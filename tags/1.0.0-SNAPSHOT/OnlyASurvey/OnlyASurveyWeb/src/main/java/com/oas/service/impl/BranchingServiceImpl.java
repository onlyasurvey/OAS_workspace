...
public class BranchingServiceImpl extends AbstractServiceImpl implements BranchingService {
	...
	private int getNextRuleApplyOrder(final Class<? extends BranchingRule> clazz, Question question) {
	...
	private Question applyExitRule(Question question, ExitRule ruleToApply) {
	...
	private Question applyEntryRules(Question question, Response response) {
		Assert.notNull(question);
	...
	@Override
	@Unsecured
	public boolean skipQuestion(Question question, Response response) {
		Assert.notNull(question);
