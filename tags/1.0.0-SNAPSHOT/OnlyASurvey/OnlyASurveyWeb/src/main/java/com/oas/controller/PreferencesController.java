...
public class PreferencesController extends AbstractOASController {
	...
	public ModelAndView doForm(HttpServletRequest request) throws Exception {
		...
		return new ModelAndView("/preferences/preferences", referenceData(request));
	}

	@Override
	protected Map<String, Object> referenceData(HttpServletRequest request) throws Exception {
		Map<String, Object> retval = new HashMap<String, Object>();
		retval.put("languageList", supportedLanguageService.getSupportedLanguages());
		...
		command.setLanguageId(((AccountOwner) getCurrentUser()).getLanguage().getId());
	...
	/**
	 * Submit the preference changes and redirect to either the default page or
	...
	@ValidUser
	public ModelAndView doSubmit(@ModelAttribute("command") PreferencesCommand command, @ModelAttribute("errors") Errors errors,
			HttpServletRequest request, HttpServletResponse response) throws Exception {

		if (isCancel(request)) {
			// user canceled
			return new ModelAndView(new RedirectView(determineRedirectUrl(command), true));
		}
		...
		SupportedLanguage userLanguage = supportedLanguageService.findById(command.getLanguageId());
	...
	public ModelAndView doChangeLanguage(HttpServletRequest request, HttpServletResponse response, String url, String rTo) {
		...
		// RedirectView(determineRedirectUrl(coalesce(url, rTo)), true));
		return createRedirect(request, coalesce(url, rTo));
	}

	private String determineRedirectUrl(PreferencesCommand command) {
		return determineRedirectUrl(command.getRedirectUrl());
	}

	private String determineRedirectUrl(String redirectUrl) {
		if (StringUtils.hasText(redirectUrl)) {
			// TODO validate not absolute, to different host, etc.
			Assert.isTrue(redirectUrl.startsWith("/"), "illegal redirect: doesn't start with a slash");
			return redirectUrl;
		} else {
			return Constants.DEFAULT_HOME;
		}
	}
}
