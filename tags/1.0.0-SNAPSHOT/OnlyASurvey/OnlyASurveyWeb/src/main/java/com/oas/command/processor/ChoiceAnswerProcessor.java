...
public class ChoiceAnswerProcessor extends AbstractAnswerProcessor {
	...
	public List<Answer> processAnswer(Survey survey, Response response, Question question, SimpleAnswerCommand command) {
		...
		// so this is an error
		Assert.isTrue((!question.isRequired()) || (question.isRequired() && !CollectionUtils.isEmpty(retval)), "invalid input: no answers");
		...
		Assert.isTrue(!((!cq.isUnlimited()) && retval.size() > 1), "invalid input: got multiple answers where only 1 (or Other Text) is allowed");
